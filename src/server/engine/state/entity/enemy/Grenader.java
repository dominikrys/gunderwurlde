...
import server.engine.ai.GrenaderAI;
...
public class Grenader extends SoldierZombie {
    ...
    //For normal creation
    public Grenader(int range_to_shoot, int rate_of_fire) {
        super(DEFAULT_HEALTH, DEFAULT_MOVESPEED, EntityList.SOLDIER, DEFAULT_SIZE, DEFAULT_DROPS, DEFAULT_SCORE_ON_KILL, new GrenaderAI(range_to_shoot, rate_of_fire));
    ...
    //For copying
    private Grenader(int maxHealth, int moveSpeed, EntityList entity, int size, LinkedHashSet<Drop> drops, int scoreOnKill, EnemyAI ai) {
    ...
    public Enemy makeCopy() {
        return new Grenader(this.maxHealth, this.moveSpeed, EntityList.GRENADER, this.size, this.drops, this.scoreOnKill, new SoldierZombieAI(rangeToShoot, rateOfFire));
    }
